// This module is a pure library of URL builders + JSON parsers consumed by the live
// orchestration in `run::run_real` (Plan 2 Task 5, out of scope here — see
// crates/ingest/src/run.rs, which is currently a stub). Everything here is exercised by
// the unit tests below but stays unreachable from `main` until Task 5 lands, so silence
// dead_code at the module level rather than deleting tested, spec'd logic.
#![allow(dead_code)]
...
mod tests {
    ...
    fn dob_record_flags_active_passenger_elevator() {
        ...
        assert!(!parse_dob_has_elevator(
            &json!({"device_type": "Elevator", "device_status": "DISMANTLED"})
        ));
    }

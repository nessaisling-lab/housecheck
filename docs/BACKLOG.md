...
- [ ] **Per-user state — accounts, saved buildings, history.** The gap in all four L2 cycles: a
      ...
      not having it.
- [ ] **Coverage past ~14,500 buildings.** Requires abandoning the baked-artifact design for a
      real database, which deletes the property the deck sells — *no DB server to breach, no write
      path to abuse.* An architectural inversion, and a decision to make deliberately.
